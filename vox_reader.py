...
class VoxObject:
    """
    Um objeto do World View do MagicaVoxel.
    
    .size    = (sx, sy, sz)  tamanho local original do modelo
    .voxels  = [(x,y,z,c)]  coordenadas LOCAIS 0-based (para o greedy_mesher)
    .offset  = (ox, oy, oz)  translação global: vertex_global = local + offset
    .name    = str
    """
    def __init__(self, name, size, voxels, offset=(0,0,0)):
        self.name   = name
        self.size   = size
        self.voxels = voxels   # coords locais
        self.offset = offset   # translação global


class VoxScene:
    """Cena completa com múltiplos objetos e paleta compartilhada."""
    def __init__(self):
        self.objects = []
        self.palette = []
    ...
    @property
    def size(self):
        return self.objects[0].size if self.objects else (0,0,0)
    @property
    def voxels(self):
        return self.objects[0].voxels if self.objects else []
...
def _apply_transform(rot, tx, ty, tz, vx, vy, vz, sx, sy, sz):
    """Aplica rotação + translação do MagicaVoxel (origem no centro do modelo)."""
...
def load_vox(path):
    """
    Carrega .vox e retorna VoxScene.
    Cada VoxObject tem voxels em coordenadas LOCAIS (0-based, para o greedy)
    e um offset global (para posicionamento no OBJ).
    """
    data = Path(path).read_bytes()
    if data[:4] != b'VOX ':
        raise ValueError(f'Não é um arquivo VOX: {path}')
    ...
    raw_models = []  # [{size, voxels_locais_originais}]
    nodes      = {}
    palette    = None
    ...
    while pos < end:
        ...
        if cid == 'SIZE':
            ...
            voxels = [(cdat[4+i*4],cdat[4+i*4+1],cdat[4+i*4+2],cdat[4+i*4+3])
            ...
            nodes[node_id]={'type':'TRN','child':child_id,
                            'tx':tx,'ty':ty,'tz':tz,'rot':rot,
    ...
    if palette is None:
        palette = _build_default_palette()

    scene = VoxScene()
    scene.palette = palette

    # Sem hierarquia: retorna modelos raw com offset 0
    if not nodes or not raw_models:
        for i, m in enumerate(raw_models):
            scene.objects.append(VoxObject(f'Object_{i}', m['size'], m['voxels']))
        return scene
    ...
    def traverse(node_id, acc_rot, acc_tx, acc_ty, acc_tz):
        ...
        if node['type'] == 'TRN':
            lr = node['rot']
            new_rot = [[sum(acc_rot[i][k]*lr[k][j] for k in range(3)) for j in range(3)]
                       for i in range(3)]
            lt = (node['tx'], node['ty'], node['tz'])
            rt = tuple(sum(acc_rot[i][k]*lt[k] for k in range(3)) for i in range(3))
            traverse(node['child'], new_rot,
                     acc_tx+rt[0], acc_ty+rt[1], acc_tz+rt[2])

        elif node['type'] == 'GRP':
            for child_id in node['children']:
                traverse(child_id, acc_rot, acc_tx, acc_ty, acc_tz)

        elif node['type'] == 'SHP':
            mid = node['model_id']
            if mid >= len(raw_models):
                return
            m = raw_models[mid]
            sx,sy,sz = m['size']
            rot = acc_rot

            # Calcula as coordenadas globais de TODOS os voxels deste objeto
            global_voxels = []
            for vx,vy,vz,color in m['voxels']:
                gx,gy,gz = _apply_transform(rot, acc_tx,acc_ty,acc_tz, vx,vy,vz, sx,sy,sz)
                global_voxels.append((gx,gy,gz,color))

            if not global_voxels:
                return

            # Offset = mínimo global de cada eixo → converte para coords locais 0-based
            ...
            # Recalcula o size local baseado nas coords locais reais
            lx_max = max(v[0] for v in local_voxels)
            ly_max = max(v[1] for v in local_voxels)
            lz_max = max(v[2] for v in local_voxels)
            local_size = (lx_max+1, ly_max+1, lz_max+1)

            name = f'Object_{obj_idx[0]}'
            obj_idx[0] += 1
            scene.objects.append(VoxObject(name, local_size, local_voxels, (ox,oy,oz)))

    identity = [[1,0,0],[0,1,0],[0,0,1]]
    root_id  = min(n for n,v in nodes.items() if v['type']=='TRN')
    traverse(root_id, identity, 0, 0, 0)
